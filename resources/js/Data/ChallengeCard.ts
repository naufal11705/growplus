import { Challenge } from "../types/challenge";

export const challenges: Challenge[] = [
    {
        id: 1,
        title: "Perjalanan Menjadi Ibu",
        subtitle: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
        deskripsi: "Mengawali perjalanan sebagai seorang ibu adalah pengalaman yang luar biasa dan penuh tantangan. Dalam tantangan harian ini, Anda akan diajak untuk memahami dan mempersiapkan setiap tahap kehamilan dengan lebih baik. Mulai dari menjaga kesehatan diri, memantau perkembangan janin, hingga memastikan asupan gizi yang optimal bagi calon buah hati. Setiap hari, Anda akan diberikan tugas sederhana namun bermanfaat, seperti memeriksa kondisi kehamilan, menerapkan pola makan sehat, serta menjaga keseimbangan emosional. Dengan menyelesaikan setiap tantangan, Anda tidak hanya memperkaya pengetahuan, tetapi juga memastikan kesejahteraan diri dan bayi yang sedang berkembang.🌱 Mulai perjalanan ini dan jadilah ibu yang siap menghadapi tantangan dengan penuh percaya diri! 💕",
        image: "https://i.ibb.co.com/KMqbPmX/brave-screenshot.png",
        tasks: [
            "Periksa ke fasilitas kesehatan jika terlambat datang bulan 1 minggu.",
            "Periksa kehamilan minimal 6 kali selama kehamilan.",
            "Makan lebih sering dan cukup minum, batasi kopi, teh, dan soda."
        ],
        benefit: [
            "Persiapan Mental dan Emosional",
            "Pola Hidup Sehat untuk Ibu dan Bayi",
            "Kesadaran akan Pemeriksaan Kehamilan",
            "Edukasi Seputar Kehamilan ",
            "Membangun Kebiasaan Positif Sejak Dini",
        ],
        progress: 10
    },
    {
        id: 2,
        title: "Pemberian (MPASI)",
        subtitle: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
                deskripsi: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
        image: "https://i.ibb.co.com/zVLq5b3D/image.png",
        tasks: [
            "Mulai MPASI pada usia 6 bulan dengan makanan yang kaya gizi.",
            "Perkenalkan berbagai jenis makanan dengan tekstur lembut.",
            "Berikan sayur dan buah setiap hari untuk memenuhi kebutuhan gizi."
        ],
        benefit: [
            "Persiapan Mental dan Emosional",
        ],
        progress: 10
    },
    {
        id: 3,
        title: "Makanan Bergizi",
        subtitle: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
                deskripsi: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
        image: "https://i.ibb.co.com/KMqbPmX/brave-screenshot.png",
        tasks: [
            "Berikan makanan yang mengandung protein, karbohidrat, dan lemak.",
            "Pastikan anak mendapatkan sumber vitamin dan mineral yang cukup",
            "Hindari memberikan makanan olahan dan cepat saji yang tinggi gula"
        ],
        benefit: [
            "Persiapan Mental dan Emosional",
        ],
        progress: 0
    },
    {
        id: 4,
        title: "Pencegahan Stunting",
        subtitle: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
                deskripsi: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
        image: "https://i.ibb.co.com/KMqbPmX/brave-screenshot.png",
        tasks: [
            "Pastikan anak mendapatkan asupan gizi yang cukup sejak dalam kandungan.",
            "Tingkatkan perkembangan gizi yang seimbang untuk anak di usia bayi/belita.",
            "Lakukan imunisasi lengkap untuk mendukung tumbuh kembang optimal."
        ],
        benefit: [
            "Persiapan Mental dan Emosional",
        ],
        progress: 0,
    },
    {
        id: 5,
        title: "Imunisasi Dasar",
        subtitle: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
                deskripsi: "Selamat! Anda kini siap memasuki tahapan baru dalam kehidupan, menjadi Ibu.",
        image: "https://i.ibb.co.com/KMqbPmX/brave-screenshot.png",
        tasks: [
            "Pastikan anak mendapatkan imunisasi dasar lengkap sesuai jadwal.",
            "Konsultasikan dengan tenaga medis jika anak mengalami efek setelah imunisasi.",
            "Jangan lupa mencatat jadwal imunisasi berikutnya agar tidak terlewat."
        ],
        benefit: [
            "Persiapan Mental dan Emosional",
        ],
        progress: 0,
    }
];
