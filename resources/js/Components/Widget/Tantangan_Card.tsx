...
const TantanganCards: React.FC<TantanganCardsProps> = ({ fases, gridCols = "xl:grid-cols-4" }) => {
    return (
        <div className={`grid gap-6 md:grid-cols-2 lg:grid-cols-2 ${gridCols} w-full`}>
            {fases.map((fase) => (
                <div key={fase.fase_id} className="bg-white rounded-2xl border-2 border-gray-100 flex flex-col h-full w-full md:max-w-sm overflow-hidden">
                    <div className="relative">
                        ...
                        <div className="absolute top-2 left-2">
                            <div className={`rounded-full p-1 ${fase.progress === 0 ? "bg-red-100" : fase.progress < 100 ? "bg-yellow-100" : "bg-light-pinky"}`}>
                                <svg className={`h-6 w-6 ${fase.progress === 0 ? "text-red-600" : fase.progress < 100 ? "text-yellow-600" : "text-wine"}`}
